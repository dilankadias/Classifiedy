      ...
      if(($offer_id > 0 || @$data['i_sell'] == 1) && (in_array($item['fk_i_user_id'], $sellers) || osp_param('seller_all') == 1)) {
        if($item['i_price'] <> '' && $item['i_price'] > 0 || $offer_id > 0) {
          ...
          if(($qty <= 0 && osp_param('stock_management') == 1 && $offer_id === null) || ($item['fk_i_user_id'] == osc_logged_user_id() && osc_is_web_user_logged_in())) {
            ...
            $html .= '<a class="osp-product-to-cart osp-disabled osp-has-tooltip ' . $class . '" title="' . $title . '" href="#" onclick="return false;">' . OSP_SVG_ADD_TO_CART . __('Buy now', 'osclass_pay') . '</a>';
          } else {
            $html .= '<a class="osp-product-to-cart ' . $class . '" href="' . osp_cart_add($ptype, $quantity, ($offer_id > 0 ? $offer_id : $item['pk_i_id']), round(osp_convert($price/1000000, $item['fk_c_currency_code']), 2)) . '">' . OSP_SVG_ADD_TO_CART . __('Buy now', 'osclass_pay') . '</a>';
          }
...
function osp_item_options($item_id) {
  ...
  if($count > 0) {
    ...
    $html .= '<a class="osp-item-to-cart" data-item="' . $item_id . '" href="' . osp_cart_add(OSP_TYPE_MULTIPLE, 1, $item_id, '') . '"><i class="fa fa-plus-circle"></i> ' . __('Buy now', 'osclass_pay') . '</a>';
    $html .= '</div>';
  } else {

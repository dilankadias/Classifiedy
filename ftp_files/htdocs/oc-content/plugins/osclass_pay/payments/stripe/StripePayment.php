...
class StripePayment {
  ...
  public static function button($amount = '0.00', $description = '', $itemnumber = '', $extra_array = null) {
    ...
    if($amount >= $min_amount) {
      ...
      try {
        // Debug output
        error_log('Currency: ' . osp_currency());
        error_log('Amount: ' . $amount);
        error_log('Zero Decimal Currency: ' . print_r($zero_decimal_cur, true));
        error_log('Order ID: ' . $order_id);
        error_log('Customer Email: ' . (osc_logged_user_email() !== '' ? osc_logged_user_email() : 'null'));
        ...
        $session = \Stripe\Checkout\Session::create([
            ...
            'shipping_address_collection' => [
                'allowed_countries' => ['AU', 'GB', 'NZ', 'US'], // Adjust this to your requirements
            ],
        ]);
    
        // Debug output
        error_log('Session created successfully: ' . print_r($session, true));
    
    } catch (\Stripe\Exception\ApiErrorException $e) {
        // Handle the exception
        error_log('Stripe API error: ' . $e->getMessage());
        echo 'There was an error creating the session: ' . $e->getMessage();
    } catch (Exception $e) {

<?php
define('BETA_THEME_VERSION', '1.4.6');
...
function bet_is_rtl() {
  ...
  if(isset($locale['b_rtl']) && $locale['b_rtl'] == 1) {
    return true;
  } else if(in_array(osc_current_user_locale(), bet_rtl_languages())) {
    return true;
  } else {
    return false;
  }
...
function bet_language_dir() {
  return bet_is_rtl() ? 'rtl' : 'ltr';
}
...
osc_add_hook('init', 'bet_ajax_manage');
...
function bet_is_lazy($disabled = false) {
  if($disabled === true) {
    return false; 
  } else if(bet_param('lazy_load') == 1 && osc_get_preference('force_aspect_image', 'osclass') == 0) {
    return true;
  }
...
function bet_profile_picture($user_id = null, $size = 'small') {
  if($user_id === null) {
...
function bet_def_location() {
  ...
  foreach($countries as $c) {
      ...
      if($counter < $limit) {
        if(strtoupper($r['fk_c_country_code']) == strtoupper($c['pk_c_code'])) {
          if($type == 'region') {
            ...
            $html .= '<div class="option region init" data-country="' . $r['fk_c_country_code'] . '" data-region="' . $r['fk_i_region_id'] . '" data-city="' . $r['pk_i_id'] . '" data-code="city' . $r['pk_i_id'] . '" id="' . $r['pk_i_id'] . '"><strong>' . osc_esc_js(osc_location_native_name_selector($r, 's_name')) . '</strong><span>' . osc_esc_js(osc_location_native_name_selector($r, 's_region_name')) . '</span></div>';
          }
        }
      }
    ...
    if($counter == $limit*count($countries)) {
      $html .= '<div class="option service empty-pick default" data-country="" data-region="" data-city="" data-code="" id=""><em>' . osc_esc_js(sprintf(__('... and %d more %s, enter your %s name to refine results', 'beta'), $limit, $type_name, $type_name)) . '</em></div>';
    }
  }
...
function bet_locbox_short($country = '', $region = '', $city = '', $level = 'all') {
  ...
  if(count($countries) > 1 && ($level == 'all' || $level == 'country')) {
    ...
    foreach($countries as $c) {
      $html .= '<div class="elem country ' . (strtoupper($c['pk_c_code']) == strtoupper($country) ? 'active' : '') . '" data-country="' . $c['pk_c_code'] . '" data-region="" data-city="" style="' . (bet_param('loc_one_row') == 1 ? 'width:' . ($box_width + 1) . 'px;'  : '') . '"><img src="' . osc_current_web_theme_url() . 'images/country_flags/large/' . strtolower($c['pk_c_code']) . '.png"/><strong>' . osc_location_native_name_selector($c, 's_name') . '</strong></div>';
    }
  ...
  if(count($categories) > 0) {
    ...
    foreach($categories as $c) {
      ...
      if($c['fk_i_parent_id'] <= 0) {
        $html .= '<div class="img">' . bet_get_cat_icon( $c['pk_i_id'] ) . '</div> <strong><span>' . $c['s_name'] . '</span></strong>';
      } else {
...
function bet_location_format($country = null, $region = null, $city = null) { 
  if($country <> '') {
    ...
    if($region <> '') {
      if($city <> '') {
        return $city . ' ' . __('in', 'beta') . ' ' . $region . (osc_location_native_name_selector($country_full, 's_name') <> '' ? ' (' . osc_location_native_name_selector($country_full, 's_name') . ')' : '');
      } else {
        return $region . ' (' . osc_location_native_name_selector($country_full, 's_name') . ')';
      }
    } else { 
      if($city <> '') {
        return $city . ' ' . __('in', 'beta') . ' ' . osc_location_native_name_selector($country_full, 's_name');
      } else {
...
if(!function_exists('bet_param_update')) {
  function bet_param_update( $param_name, $update_param_name, $type = NULL, $plugin_var_name = null ) {

...
<html xmlns="http://www.w3.org/1999/xhtml" dir="<?php echo bet_language_dir(); ?>" lang="<?php echo str_replace('_', '-', osc_current_user_locale()); ?>">
<head>
  ...
  <?php
    ...
    if(osc_item_show_phone() == 0) {
      ...
      $mobile = __('Login to see phone number', 'beta');
      $mobile_found = true;
...
<body id="body-item" class="page-body<?php if($itemviewer == 1) { ?> itemviewer<?php } ?><?php if(bet_device() <> '') { echo ' dvc-' . bet_device(); } ?>">
    ...
    <div class="side">
      ...
      <div class="data">
          ...
          <?php if($mobile_found) { ?>
            <div class="row mob">
              ...
              <?php if($mobile_login_required) { ?>
                <a href="<?php echo osc_user_login_url(); ?>" class="mobile login-required" data-phone="" title="<?php echo osc_esc_html(__('Login to show number', 'beta')); ?>"><?php echo __('Login to show number', 'beta'); ?></a>
              <?php } else { ?>
        ...
        <div class="item-share">
          ...
          <a class="pinterest" title="<?php echo osc_esc_html(__('Share on Pinterest', 'beta')); ?>" target="_blank" href="https://pinterest.com/pin/create/button/?url=<?php echo urlencode(osc_item_url()); ?>&media=<?php echo urlencode(osc_resource_url()); ?>&description=<?php echo htmlspecialchars(osc_item_title()); ?>"><i class="fa fa-pinterest"></i></a> 
        </div>
      </div>
  ...
  <?php } else { ?>
    <div class="mobile-item-data" style="display:none">
      <a href="<?php echo osc_user_login_url(); ?>"><?php _e('Login to show phone number', 'beta'); ?></a>
    </div>
  <?php } ?>
